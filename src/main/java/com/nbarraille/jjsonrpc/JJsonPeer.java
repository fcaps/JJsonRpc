...
public class JJsonPeer extends Thread {
    ...
    private final static Logger _log = org.slf4j.LoggerFactory.getLogger(JJsonPeer.class);
    ...
    public void run() {
        try {
            _log.info("JJSON Peer listening");
            int b;
            ...
            while ((b = _in.read()) != -1) {
                ...
                if (b == '{') {
                    ...
                    //_log.info("Peer on port:" + _socket.getPort() + " said: " + data);
                    // Process data
            ...
            _log.info("Connection has been closed (" + e.getMessage() + ")");
        ...
        try {
            ...
            try {
                ...
                try {
                    ...
                    _log.info("The data received cannot be parsed : " + data);
    ...
    private void processRequest(JSONRPC2Request req) {
        ...
        try {
            ...
            _log.info("Invalid Request: Cannot retrieve ID");
    ...
    private void processNotification(JSONRPC2Notification not) {
        ...
        if (m == null) {
            ...
            _log.info("Method not found : " + method + " with params " + Arrays.asList(params));
            return;
        }
        ...
        try {
            ...
            _log.info("A Security Manager prevented the access to this method");
        } catch (IllegalArgumentException e) {
            // The method was called with the wrong number/types of arguments, SHOULDNT HAPPEN
            e.printStackTrace();
        } catch (IllegalAccessException e) {
            // Java Language Access prevented the invocation of this method
            _log.info("Java Language Access prevented the invocation of this method");
        } catch (InvocationTargetException e) {
            // The method has thrown an exception
        }
    }
    ...
    private void processResponse(JSONRPC2Response resp) {
        ...
        try {
            ...
            _log.info("Invalid Response: Cannot retrieve ID");
            return;
        }
    ...
    public void sendNotification(String methodName, List<Object> args) {
        ...
        _log.info("Sending Notification:" + s);
        synchronized (_out) {
            _out.println(s);
            _out.flush();
        }
    ...
    public void sendErrorResponse(int code, String message, Long reqId) {
        ...
        _log.info("Sending Error Response:" + s);
        synchronized (_out) {
            _out.println(s);
            _out.flush();
        }
